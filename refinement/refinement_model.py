...
class RefinementModel():
    ...
    def _refine_explanation(self, original_explanation: str,
                            llm_assumption: str,
                            hypothesis: str,
        ...
        print('-----------refined llm assumption--------------------')
        print(refined_explanations)
        print('--------------------------------------------------')
    ...
    def refine(self, hypothesis: str, premise: Optional[str] = None,
               ...
               llm_assumption: Optional[str] = None,
        ...
        for i in range(iterations):
            ...
            print('-----------llm assumption--------------------')
            for m, sentence in enumerate(llm_assumption.split('.'), 1):
                if sentence.strip():
                    print((f"{m}. {sentence.strip()}"))
            print('--------------------------------------------------')
            to_append_explanation = f'{i} iteration: {explanation}'
            to_append_llm_assumption = f'{i} iteration: {llm_assumption}'
            history_explanation.append(to_append_explanation) 
            history_llm_assumption.append(to_append_llm_assumption)
            ...
            if self.critique_mode == 'hard':
                if not critique_output['syntactic validity']:
                    ...
                    if i == iterations - 1:
                        ...
                        result = {
                            ...
                            'llm assumption': llm_assumption,
                            'history explanation': history_explanation,
                            'history llm assumption': history_llm_assumption,
                ...
                if critique_output['semantic validity']:
                    ...
                    llm_assumption = self._refine_explanation(
                        original_explanation, llm_assumption,
                        hypothesis, critique_output, premise,
                    )
                    explanation = original_explanation + "\n" + llm_assumption
                    print('\n-------------- 6) Refined LLM Assumption -------------------\n')
                    for n, sentence in enumerate(llm_assumption.split('.'), 1):
                        if sentence.strip():
                            print(f"{n}. {sentence.strip()}")
                    print('----------- 7) Refined explanation --------------------')
                    for p, sentence in enumerate(explanation.split('.'), 1):
                        if sentence.strip():
                            print((f"{p}. {sentence.strip()}"))
                    print('--------------------------------------------------')
                    if i == iterations - 1:
                        print(f'The explanation is not valid after {iterations} iterations')
                        result = {
